...
1. The brief, exactly as written, nothing added:
   ```prompt
   Write a report on the future of tourism in Cyprus.
   ```
2. Run it once in the research mode of claude.ai (Codex: chatgpt.com, deep research). Wait for it to finish.
3. Save the report as `report1.md` in the `bpa347-week3` folder on the Desktop. Copy the text into a file if there is no download.
4. Read it. Mark every place where it says what it could not find.
