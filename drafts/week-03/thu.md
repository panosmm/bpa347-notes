...
- Ask the model, in an ordinary chat, to interview you before you send the request. It stops asking before it has everything: when it writes the request, ask what it still leaves for the research agent to decide, and answer that too. Its last message is the full request; give that to the research agent.
...
1. The interview. Open claude.ai (Codex: chatgpt.com), start a new chat and paste this:
   ```prompt
   I am going to give a research agent this request: "Plan a budget trip to Rome for me and a friend, 4 or 5 days in the week before Christmas. We have 600 euros each for everything. Decide everything else yourself." Before I do, interview me instead: ask me one question at a time about everything the request leaves for the research agent to decide, then write the full request as one message, ready to paste.
   ```
2. Answer the questions. When the model writes the full request, do not send it yet. Push for more, and answer again:
   ```prompt
   What does this request still leave for the research agent to decide? Ask me about all of it, one question at a time, then write the full request again.
   ```
   Repeat until it has nothing more to ask.
3. Copy the full request. Start a new chat, click `+` and choose **Research** (Codex: **Deep research**), paste the request and send it. The research agent runs for some minutes.
4. While it runs, save the request. Go back to the interview chat and click **Copy** under the model's last message. Save it as `request.md` in the `bpa347-week3` folder. Windows: paste into Notepad, click **File**, then **Save as**, choose **All files** under "Save as type", type `request.md` as the file name, go to the folder and click **Save**. Mac: open TextEdit, click **Format**, then **Make Plain Text**, paste, click **File**, then **Save**, type `request.md` as the name, go to the folder, click **Save**, then **Use .md**.
5. When the research agent finishes, save the report as `report2.md` in the same folder, the same way. Codex: click the share icon above the report, then **Download**, then **Markdown**; move the file from Downloads into the folder and rename it `report2.md`.
6. Which report answers the request? Open a terminal in the `bpa347-week3` folder, start the agent and paste this:
   ```prompt
...
7. Two agents. Type `/exit`. Open a second terminal in the same folder. Start the agent in the first terminal with the name author, and in the second with the name auditor:
   ```bash
   claude --name author
   ```
   ```bash
   ...
   ```
8. In the author's terminal, paste this:
   ```prompt
   ...
   ```
9. In the auditor's terminal, paste this:
   ```prompt
   ...
   ```
10. Read the auditor's table. In the author's terminal, a message from the auditor shows as one line; press `Ctrl+O` to see it in full. Where the author agreed that a claim does not hold, ask the author to correct the report. Where the two disagree, you decide.
